		...
		stage('Test')
			{
				steps{
						...
						sh 'docker run -v /var/jenkins_home/workspace/prueba2iib:/etc/newman -t --name postmantemp postman/newman_ubuntu1404 --collection="HTTPBinNewmanTest.json.postman_collection"'						
						sh 'docker rm postmantemp'
					}
